...
Nawdli Express release APKs must be signed with the same keystore every time.
...
```text
android/app/nawdli-express-release-key.jks
...
```powershell
[Convert]::ToBase64String([IO.File]::ReadAllBytes("android\app\nawdli-express-release-key.jks")) | Set-Content keystore_base64.txt
```
